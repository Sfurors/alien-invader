...
def draw_frame(screen, rts_ctx, state, fog, minimap_renderer):
    ...
    # Draw terrain
    _draw_terrain(screen, rts_ctx.tile_map, camera, fog)

    # Draw buildings (sorted by y for overlap)
    _draw_buildings(screen, rts_ctx, camera, fog)

    # Draw units
    _draw_units(screen, rts_ctx, camera, fog)

    # Draw projectiles
    _draw_projectiles(screen, rts_ctx, camera)

    # Fog overlay
    fog.draw_overlay(screen, camera)

    # Build preview
    if state.build_mode and state.build_preview_pos:
        _draw_build_preview(screen, state, camera)
...
def _draw_units(screen, rts_ctx, camera, fog):
    ...
    for unit in all_units:
        ...
        if unit.can_harvest and unit.harvesting and not unit.path:
            ...
            pygame.draw.rect(
                screen, (0, 200, 220), (bar_sx, bar_sy, int(bar_w * progress), bar_h)
            )
...
def _draw_rock_tile(screen, sx, sy, ts, tx, ty):
    ...
    # Deterministic variation per tile
    seed = tx * 31 + ty * 17
    ...
    # Rock cracks/texture based on seed
    cx1 = sx + 6 + (seed % 11)
    cy1 = sy + 5 + (seed % 7)
    cx2 = cx1 + 4 + (seed % 8)
    cy2 = cy1 + 6 + (seed % 6)
...
def _draw_hud(
    ...
    if total_cap > 0:
        crystal_text = font.render(
            f"Crystals: {state.crystals}/{total_cap}", True, (80, 220, 255)
        )
    else:
    ...
    if state.selected_units:
        count = len(state.selected_units)
        types = {}
        ...
        screen.blit(sel_text, (info_x, hud_y + 35))
        ...
        screen.blit(b_text, (info_x, hud_y + 35))

        hp_text = small_font.render(f"HP: {b.hp}/{b.max_hp}", True, (200, 200, 200))
        screen.blit(hp_text, (info_x, hud_y + 55))
        ...
        if b.produces:
            ...
            for i, utype in enumerate(b.produces):
                ...
                prod_text = small_font.render(
                    f"[{hotkey}] {utype} ({cost}c)", True, (180, 180, 200)
                )
                screen.blit(prod_text, (info_x, prod_y + i * 18))

        if b.production_queue:
            q_text = small_font.render(
                f"Queue: {', '.join(b.production_queue)}", True, (150, 150, 200)
            )
            screen.blit(q_text, (info_x, hud_y + 115))
    ...
    if state.build_mode:
        ...
        if has_engineer:
            hint = small_font.render(
                "[1] Base  [2] Barracks  [3] Turret", True, (150, 150, 150)
            )
            screen.blit(hint, (info_x, hud_y + 140))
